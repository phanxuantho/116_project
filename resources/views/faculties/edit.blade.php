<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Chỉnh sửa Khoa: ') . $faculty->faculty_name }}
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('faculties.update', $faculty->id) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                {{-- Mã khoa thường hạn chế sửa, nhưng nếu cho sửa thì bỏ readonly --}}
                                <x-text-input id="faculty_code" class="block mt-1 w-full" type="text" name="faculty_code" :value="old('faculty_code', $faculty->faculty_code)" required />
                                <x-input-error :messages="$errors->get('faculty_code')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="faculty_name" :value="__('Tên Khoa')" />
                                <x-text-input id="faculty_name" class="block mt-1 w-full" type="text" name="faculty_name" :value="old('faculty_name', $faculty->faculty_name)" required />
